...
(function() {
			...
			ed.addCommand('mcePasteWord', function(ui, v) {
				if (ui) {
					if ((ed.getParam('paste_use_dialog', true)) || (!tinymce.isIE)) {
						...
						*/
						new $K.yg_wndobj( { config: 'CONTENTEDITOR_PASTEWORD' });
						/*new Ajax.Updater('dialogcontainer', $K.appdir+'window?wt=modal&display=cepasteword', {
							asynchronous: true,
							evalScripts: true,
							method: 'post',
							insertion: 'bottom',
	...
	// Register plugin
	tinymce.PluginManager.add('yeager_paste', tinymce.plugins.PastePlugin);
})();