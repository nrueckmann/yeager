...
(function() {
		...
		_doExecCommand : function(command, user_interface, value) {
			...
			switch (command) {
				...
				case "mceTableRowProps":
					...
					*/
					new $K.yg_wndobj( { config: 'CONTENTEDITOR_TABLEROW' });
				...
				case "mceTableCellProps":
					...
					*/
					new $K.yg_wndobj( { config: 'CONTENTEDITOR_TABLECELL' });
					return true;

				case "mceInsertTable":
					new $K.yg_wndobj( { config: 'CONTENTEDITOR_TABLE' });
					return true;
							...
							if (tinymce.isIE || sel.rangeCount == 1) {
								if (user_interface) {
									...
									*/
									...
									new $K.yg_wndobj({ config: 'CONTENTEDITOR_MERGECELLS' });
									return true;
								} else {
	...
	// Register plugin
	tinymce.PluginManager.add('yeager_table', tinymce.plugins.TablePlugin);
})();