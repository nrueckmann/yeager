...
function nls_addNodeXML(tree, prnId, xnd) {
  for (var i=0; i<xnd.childNodes.length; i++) {
    ...
    var newNd=tree.add(nd.getAttribute("id"), prnId, nd.getAttribute("caption"), nd.getAttribute("url"), nd.getAttribute("ic"), nd.getAttribute("exp")=="true", nd.getAttribute("chk")=="true", nd.getAttribute("xtra"), nd.getAttribute("title"), null, nd.getAttribute("yg_id"), nd.getAttribute("yg_type"), nd.getAttribute("yg_property"), nd.getAttribute("pname"));
	...
	if ((newNd.cstStyle.indexOf('nosub')!=-1) && (newNd.yg_type!='page')) {
		// if no sub permissions
		tree.setDrop(nd.getAttribute("id"), false, false);
	} else {
		if (!tree.opt.sortable) {
			// if not sortable disable drag & drop within tree
			tree.setDrop(nd.getAttribute("id"), false, false);
		}
	}
	if (newNd.cstStyle.indexOf('nowrite')!=-1) {
		// if no write permissions
		tree.setEditablity(nd.getAttribute("id"), false);
		/* tree.setDrag(nd.getAttribute("id"), false, false); */
	} else if (tree.opt.editable) {
		// if editable activate inline editability
		tree.setEditablity(nd.getAttribute("id"), true);
	}

   	if (nd.getAttribute("dnd") == 'false') {
		nls_setNodeDnD(tree, nd.getAttribute("id"), "allowDrag", false, false);
	}
...
NLSTREE.addChildNodesXML=function(rn, reload, updateParent) {
  ...
  if (updateParent) {
	...
	nd.yg_id=rn.getAttribute("yg_id");
	nd.yg_type=rn.getAttribute("yg_type");
	nd.yg_property=rn.getAttribute("yg_property");
	nd.pname=rn.getAttribute("pname");
	if (rn.getAttribute("dnd") == 'false') {
		nls_setNodeDnD(this, rn.getAttribute("id"), "allowDrag", false, false);
	}
