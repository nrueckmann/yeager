...
function NlsTree(tId) {
  ...
  for (var i=0;i<asc.length;i++) {
    if (asc[i].src.toLowerCase().indexOf("nlstree.js")>=0) {this.defImgPath=asc[i].src.replace(/nlstree.js/gi, "../../img/tree/");break;}
  }
...
/*  this.pnb = path+"plusnb.gif";
  ...
  this.opf = path+"blank.gif";
  this.clf = path+"blank.gif";
  this.chd = path+"blank.gif";
  this.rot = path+"blank.gif";
...
function NlsNode(orgId, capt, url, ic, exp, chk, xtra, title, add, yg_id, yg_type, yg_property, pname) {
  this.orgId = orgId;
  this.id = "";
  this.capt = capt;
  this.url = null;
  this.ygurl = (url==null || url=="") ? "" : url;
  this.ic = null;
  ...
  this.yg_id = (yg_id?yg_id:null);
  this.yg_type = (yg_type?yg_type:null);
  this.yg_property = (yg_property?yg_property:null);
  this.pname = (pname?pname:null);
...
//NLSTREE.compareNode = function(aN, bN) { return (aN.capt.toLowerCase() >= bN.capt.toLowerCase()); };
NLSTREE.compareNode = function(aN, bN) {
	var result = (aN.capt.toLowerCase() >= bN.capt.toLowerCase());
	if (aN.ic && bN.ic && (aN.ic[0].indexOf('ico_folder') != -1) && (bN.ic[0].indexOf('ico_folder') != -1)) {
		result = (aN.capt.toLowerCase() > bN.capt.toLowerCase());
	} else if (aN.ic && aN.ic[0].indexOf('ico_folder') != -1) {
		result = false;
	} else if (bN.ic && bN.ic[0].indexOf('ico_folder') != -1) {
		result = true;
	}
	if (aN.ic && aN.ic[0].indexOf('ico_trashcan') != -1) {
		result = true;
	} else if (bN.ic && bN.ic[0].indexOf('ico_trashcan') != -1) {
		result = false;
	}
	return result;
};

NLSTREE.add = function(id, prn, capt, url, ic, exp, chk, xtra, title, add, yg_id, yg_type, yg_property, pname) {
  var nNd = new NlsNode(((id==null||String(id)=="")?("int"+ (++this.nCnt)):id), capt, url, ic, exp, chk, xtra, title, add, yg_id, yg_type, yg_property, pname);
  ...
  // Add to lookuptable if we have special properties
  if (yg_id && yg_type && yg_property)
  	$K.yg_addLookup( yg_id, nNd );
...
NLSTREE.addBefore = function (id, sib, capt, url, ic, exp, chk, xtra, title, add, yg_id, yg_type, yg_property, pname) {

  var nd=this.getNodeById(sib);
  if (nd==null) return;

  var nNd = new NlsNode(((id==null||id=="")?("int"+ (++this.nCnt)):id), capt, url, ic, exp, chk, xtra, title, add, yg_id, yg_type, yg_property, pname);
  nNd.id = this.genIntId(nNd.orgId);
...
NLSTREE.addAfter = function (id, sib, capt, url, ic, exp, chk, xtra, title, add, yg_id, yg_type, yg_property, pname) {
  var nd=this.getNodeById(sib);
  if (nd==null) return;
...
NLSTREE.append = function(id, prn, capt, url, ic, exp, chk, xtra, title, add, yg_id, yg_type, yg_property, pname) {
  var nd = this.add(id, prn, capt, url, ic, exp, chk, xtra, title, add, yg_id, yg_type, yg_property, pname);
  this.reloadNode(prn);
  return nd;
};
...
function DefRenderer(tId) {
  ...
  this.tr=tr;
  this.rat=new Object();
  this.rat_noclick=new Object();
  this.dsp=new Object();
  
  this.initRender = function() {
    var opt=tr.opt; var ico=tr.ico;
    
    var ev="onclick=\"if($K.yg_selectNode(this,event)) return nls_c2(event, '"+tId+"','@id')\" ";
    ...
    if (Prototype.Browser.IE) ev+="ondblclick=\"return nls_c2(event, '"+tId+"','@id')\" ";

    // Checkcheck
    //if (opt.evMouseOver) ev+="onmouseover=\"return nls_c7(event, '"+tId+"', '@id')\" ";
	//if (opt.evMouseOver) ev+="onmouseover=\"if ((this._onmouseover!=undefined)&&(typeof(this._onmouseover)=='function')) this._onmouseover(event); return nls_c7(event, '"+tId+"', '@id')\" ";    
	ev+="onmouseover=\"if ((this._onmouseover!=undefined)&&(typeof(this._onmouseover)=='function')) this._onmouseover(event); return nls_c7(event, '"+tId+"', '@id')\" ";
	var evho="onmouseover=\"if ((this._onmouseover!=undefined)&&(typeof(this._onmouseover)=='function')) this._onmouseover(event); return nls_c7(event, '"+tId+"', '@id')\" ";
    ...
    var ex=["<img src='@expdr' onclick=\"nls_c1(event, '"+tId+"', '@id')\" onmouseover=\"nls_c11(event, '"+tId+"', '@id')\" onmouseout=\"nls_c12(event, '"+tId+"', '@id')\">", "<img src='@expdr'>", "<img class=\"tree_ico\" src='@ic'>"];
    ...
    this.rat["cnt"]=["<div "+ev+" id='@id' class='"+opt.stlprf+"row' title=\"@title\">", "<div id='@id' class='"+opt.stlprf+"row' title=\"@title\" style='display:none'>", "</div><div style='display:block' id='ch_@id'>", "</div><div style='display:none' id='ch_@id'>"];

    this.rat_noclick["cnt"]=["<div "+evho+" id='@id' class='"+opt.stlprf+"row noclick' title=\"@title\">", "<div id='@id' class='"+opt.stlprf+"row' title=\"@title\" style='display:none'>", "</div><div style='display:block' id='ch_@id'>", "</div><div style='display:none' id='ch_@id'>"];
    ...
    this.rat["mn"]=["<table cellspacing=0 cellpadding=0 border=0><tr><td nowrap>","</td><td nowrap>", "</td><td nowrap>","</td><td nowrap>", "</td><td width='100%'>", "</td></tr></table>"];
    this.rat["ics"]=ex[2];
    ...
    this.rat["ac"]=["<a class=\"@nstyle\" style='display:block'>@capt</a>", "<a style='display:block;' target='@trg' href=\""];
  ...
  this.genNodes = function(sNd, incpar, wrt) {
    var s="";
    if (incpar) {
      //s=this.rat["cnt"][(tr.opt.hideRoot && sNd.equals(tr.rt)?1:0)]+this.genANode(sNd)+this.rat["cnt"][(sNd.fc && sNd.exp?2:3)];
      if (sNd.xtra == 'noclick') {
    	s=this.rat_noclick["cnt"][(tr.opt.hideRoot && sNd.equals(tr.rt)?1:0)]+this.genANode(sNd)+this.rat["cnt"][(sNd.fc && sNd.exp?2:3)];
      } else {
        s=this.rat["cnt"][(tr.opt.hideRoot && sNd.equals(tr.rt)?1:0)]+this.genANode(sNd)+this.rat["cnt"][(sNd.fc && sNd.exp?2:3)];
      }
    };
...
DefRenderer.$genNodes=function(sNd, incpar, wrt) {
  ...
  while(chNode) {
    if(chNode.exp) {
      ...
      if (chNode.xtra == 'noclick') {
        tmp=this.rat_noclick["cnt"][0]+this.genANode(chNode)+this.rat["cnt"][(chNode.fc && chNode.exp?2:3)]+"</div>";
      } else {
        tmp=this.rat["cnt"][0]+this.genANode(chNode)+this.rat["cnt"][(chNode.fc && chNode.exp?2:3)]+"</div>";
      }
...
function $getAnchor(nd){if(nd) return nd.childNodes[0].childNodes[0].childNodes[0].childNodes[3].childNodes[0]}
...
NLSTREE.reloadNode = function(id, incChd) {
  ...
  if (cNode.exp==true && cNode.fc!=null && dvN) dvN.style.display="";
  ...
  if (this.selNd!=null) {var sId=this.selNd.id; this.selNd=null; this.selElm=null; this.selectNode(sId, true); }
};
...
function nls_setStyle(tree, sNd, sElm, selected) {
  ...
  if ((tree.opt.selRow) && (sElm != null)) sElm.className = tree.opt.stlprf+prf+"row";
  ac = $getAnchor(sElm);
  if (ac) ac.className = (sNd.cstStyle!=""?prf+sNd.cstStyle:tree.opt.stlprf+prf+($pbsHasChild(sNd)?"prnnode":"node"));
};
...
function nls_setNodeIcon(tree, sNd, sElm, selected) {
  if (tree.opt.icon) { 
    ...
    if (selected) {
      if (sNd.ic!=null) {
    	  if (nlsTreeIc[sNd.ic[1]?sNd.ic[1]:sNd.ic[0]]) {
    		  ic.src = nlsTreeIc[sNd.ic[1]?sNd.ic[1]:sNd.ic[0]].src
    	  }
      } else {
    	  ic.src=nlsTreeIc[(sNd.id==tree.rt.id?tree.ico.rot:($pbsHasChild(sNd)?tree.ico.opf:tree.ico.chd))].src;
      }
      if(ic.fixpng) ic.fixpng();
    } else {
...
NLSTREE.selectNode = function (id, noTimeOut) { /*id is internal id*/
  nlsTree.selectedTree=this;
  if (this.opt.editable) {
    var sNd=this.selNd;
    
    if (!NlsTree._blockEdit) {
	    if (this.liveNodeEditClicked && (this.liveNodeEditClicked == id) ) {
			this.liveNodeEditStart(id);
    	}
	}
    if (!noTimeOut) {
        this.liveNodeEditClicked = id;
        setTimeout("nlsTree."+this.tId+".liveNodeEditClicked = null;", 400);
    } else {
    	this.liveNodeEditClicked = null;
    }
  ...
  if (this.opt.mntState && nls_setCookie && sNd) nls_setCookie(this.tId+"_selnd", sNd.orgId);
...
NLSTREE.selectNodeById = function(id) {
  ...
  this.selectNode(node.id, true);  /*select the node*/ 
};
...
NLSTREE.expandNodes = function(ids) {
  for(var i=0;i<ids.length;i++) {
	  var sNd = this.getNodeById(ids[i]);
	  if (!sNd.exp && sNd.fc) {
	  	this.toggleNode(sNd.id);
	  } else if (sNd.svrLoad && sNd.loaded==0) {
	  	this.toggleNodes( ids );
	    break;
	  }
  }
};
...
NLSTREE.toggleNodes = function(ids) {
  this.ajaxLoadChildNodesList( ids );return;
};
...
NLSTREE.setNodeCaption = function(id, capt) {
  ...
  if (this.opt.sort == 'no') {
	  nd.innerHTML = capt;
  } else if (this.opt.sort == 'asc') {
	  
	  var firstItem = sNd.pr.fc;
	  var lastItem = sNd.pr.lc;
	  
	  var nodesArray = new Array();
	  var currNode = firstItem; 
	  while (currNode != lastItem) {
		  if (currNode.ic && currNode.ic[0].indexOf('ico_folder') != -1) {
			  nodesArray.push( '__________'+currNode.capt.toLowerCase()+'<<<>>>'+currNode.orgId );
		  } else {
			  nodesArray.push( currNode.capt.toLowerCase()+'<<<>>>'+currNode.orgId );
		  }
		  currNode = currNode.nx;
	  }
	  if (lastItem.ic && lastItem.ic[0].indexOf('ico_folder') != -1) {
		  nodesArray.push( '__________'+lastItem.capt.toLowerCase()+'<<<>>>'+lastItem.orgId );
	  } else {
		  nodesArray.push( lastItem.capt.toLowerCase()+'<<<>>>'+lastItem.orgId );
	  }
	  nodesArray.sort();
	  
	  var newIdx = nodesArray.indexOf( sNd.capt.toLowerCase()+'<<<>>>'+sNd.orgId );
	  
	  if (newIdx == -1) {
		  newIdx = nodesArray.indexOf( '__________'+sNd.capt.toLowerCase()+'<<<>>>'+sNd.orgId );
	  }
	  
	  if ( (nodesArray.length==1) || ((newIdx==0)&&(sNd==firstItem)) ) {
		  nd.innerHTML = capt;  
	  } else if (newIdx==0) {
		  this.ctx_moveChild([sNd], firstItem, 2);
		  this.selectNode( intId, true );
	  } else if (nodesArray.length>1) {
		  var prevNodeId = nodesArray[newIdx-1].split('<<<>>>')[1];
		  var prevNodeIdInt = this.genIntId(prevNodeId);
		  var prevNode = this.nLst[prevNodeIdInt]; 
		  this.ctx_moveChild([sNd], prevNode, 3);
		  this.selectNode( intId, true );
	  }
	  
  } else if (this.opt.sort == 'desc') {
	  /*
	  var intId = this.genIntId(id);
	  var nd = $getAnchor(NlsGetElementById(intId));
	  var sNd = this.nLst[intId];
...
NLSTREE.liveNodeWrite = function() {
  ...
  if (edt.value != "" && edt.value!=this.selNd.capt) {
	  if (this.treeOnBeforeNodeChange(this.selNd.orgId, this.tId)) {
		  var oldSelNd = this.selNd;
		  this.setNodeCaption( this.selNd.orgId, edt.value );
		  /*
		  ac.innerHTML=edt.value;
		  this.selNd.capt=edt.value;
		  */
		  this.selNd = oldSelNd;
		  this.treeOnNodeChange(this.selNd.orgId, this.tId);
		  this.selectNodeById(this.selNd.orgId);
	  } else {
		  return;
	  }
  }
...
function nls_c2(e, tId, nId){
  ...
  if (e && (e.ctrlKey||e.metaKey) && e.altKey) { if (e.stopPropagation) {e.stopPropagation();} else {e.cancelBubble=true}; return t.contextMenu(e, nId); }
...
function nls_c6(e, tId, nId) {
  ...
  if ( (target.tagName=='IMG') && (target.className!='tree_ico') ) return;
